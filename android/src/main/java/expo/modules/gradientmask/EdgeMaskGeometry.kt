...
internal object EdgeMaskGeometry {
    ...
    fun containsTouch(y: Float, top: Double, bottom: Double, container: Float, density: Float, topRatio: Boolean = false, bottomRatio: Boolean = false): Boolean {
        val start = height(top, topRatio, container, density)
        val end = maxOf(start, height(bottom, bottomRatio, container, density))
